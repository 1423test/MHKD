...
def evaluate(embeds, ratio, idx_train, idx_val, idx_test, label, nb_classes, device, dataset, lr, wd
    ...
    for _ in range(50):
        ...
        # if isTest and test_micro_f1s[max_iter] > best:
        #     best = test_micro_f1s[max_iter]
        #     f = open("./embeds/" + dataset + "/" + 'logits_'+ str(ratio) + ".pkl", "wb")
        #     pkl.dump(all_logits.cpu().data.numpy(), f)
        #     f.close()
