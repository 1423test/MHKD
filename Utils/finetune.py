import numpy as np
import torch
from teacher.HeCo.code.utils.logreg import LogReg
...
def finetune(embeds, ratio, idx_train, idx_val, idx_test, label, nb_classes, device, dataset, lr, wd,Ln,Lg,norm
             , isTest=True):
    ...
    for _ in range(50):
        ...
        for iter_ in range(200):
            ...
            loss_ce = xent(logits, train_lbls)

            h_s = log(embeds)
            loss_kd = loss(h_t, h_s, Ln, Lg, norm)
            loss_train = loss_ce + loss_kd
